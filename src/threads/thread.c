...
/*new*/
  list_insert_ordered(&ready_list,&t->elem,thread_pri_less,NULL);
  t->status = THREAD_READY;
  intr_set_level (old_level);
/*new*/
...
thread_yield (void) 
{
  ...
  list_insert_ordered (&ready_list, &cur->elem,thread_pri_less,NULL);   
...
thread_set_priority_other(struct thread *curr,int new_priority,bool forced)
{
	...
	list_remove(&curr->elem);
        list_inserted_ordered (&ready_list, &curr->elem , pri_more, NULL);
    }
    else if(list_entry(list_begin(&ready_list),struct thread,elem)->priority>=new_priority)
    {
        thread_yield();
    }
...
thread_set_priority_other (struct thread *curr, int new_priority, bool forced)
{
 ...
 if (curr->status == THREAD_READY) /* Re-order the ready-list */
    {
      list_remove (&curr->elem);
      list_insert_ordered (&ready_list, &curr->elem, thread_pri_less, NULL);
    }
...
{
   int pri;
   pri = PRI_MAX-float_to_int2(float_div_int(t->recent_cpu,4))-(t->nice*2);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_insert_ordered (&all_list, &t->allelem,thread_pri_less,NULL);
}
